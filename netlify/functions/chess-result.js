...
export async function handler(event) {
  ...
  if (!profileId || !gameId || !["white", "black"].includes(playerColor)) {
    return json(400, {
  ...
  const isPlayerInGame =
    game.white_player === profileId ||
    game.black_player === profileId;
  ...
  const nextProfile = {
    elo: didWin
      ? Number(profile.elo || 0) + 25
      : Math.max(0, Number(profile.elo || 0) - 10),
    wins: didWin
      ? Number(profile.wins || 0) + 1
      : Number(profile.wins || 0),
    losses: didWin
      ? Number(profile.losses || 0)
      : Number(profile.losses || 0) + 1,
